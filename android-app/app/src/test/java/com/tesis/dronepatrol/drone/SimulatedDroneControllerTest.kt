...
class SimulatedDroneControllerTest {
    ...
    @Test
    fun elThrottleSubeAlDron() = runBlocking {
        dron.connect()
        val antes = withTimeout(5_000) { dron.telemetry.first() }

        dron.manualStick(pitch = 0.0, roll = 0.0, yaw = 0.0, throttle = 1.0)
        delay(1_500) // a 2 m/s tendría que haber subido unos 3 m
    ...
    @Test
    fun elYawGiraElRumbo() = runBlocking {
        dron.connect()
        // Arranca mirando al norte y gira en sentido horario a 45°/s
        dron.manualStick(pitch = 0.0, roll = 0.0, yaw = 1.0, throttle = 0.0)
        delay(1_500)

        val rumbo = withTimeout(5_000) { dron.telemetry.first() }.heading
        assertTrue("rumbo $rumbo: tendría que haber girado hacia el este", rumbo in 30.0..110.0)
    }

    @Test
    fun elPitchLoMueveHaciaDondeMiraLaCamara() = runBlocking {
        dron.connect()
        // Con la nariz al norte (rumbo inicial 0), adelante es al norte
        dron.manualStick(pitch = 1.0, roll = 0.0, yaw = 0.0, throttle = 0.0)
        delay(1_500)
    ...
    @Test
    fun conLosCuatroEjesEnCeroSeQuedaQuieto() = runBlocking {
        dron.connect()
        dron.manualStick(pitch = 1.0, roll = 0.0, yaw = 0.0, throttle = 1.0)
        delay(1_000)
        dron.manualStick(pitch = 0.0, roll = 0.0, yaw = 0.0, throttle = 0.0)
        delay(700) // margen para que termine el tick en curso

        val t1 = withTimeout(5_000) { dron.telemetry.first() }
        delay(1_500)
        val t2 = dron.telemetry.first()
        assertEquals(t1.lat, t2.lat, 1e-12)
        assertEquals(t1.lon, t2.lon, 1e-12)
        assertEquals(t1.altM, t2.altM, 1e-12)
    }

    /**
     * La altura del mando no se le queda pegada al patrullaje: en cuanto el dron
     * vuelve a volar solo, sube o baja a la de crucero (40 m).
     */
    @Test
    fun alSalirDelMandoLaAlturaVuelveALaDeCrucero() = runBlocking {
        dron.connect()
        dron.manualStick(pitch = 0.0, roll = 0.0, yaw = 0.0, throttle = -1.0)
        delay(2_000)
        val bajo = withTimeout(5_000) { dron.telemetry.first() }.altM
        assertTrue("tendría que haber bajado de los 40 m, quedó en $bajo", bajo < 39.0)

        dron.hold()
        delay(3_000)

        assertEquals(40.0, dron.telemetry.first().altM, 0.5)
    }
